const comnputer_element = document.getElementById("computer");
...
function rotateFonts(){
    ...
    comnputer_element.style.fontFamily = `${nextFont}`;
    prevFontIndex = fontIndex;
}

function rotatingHeader(){
    rotationInterval = setInterval(rotateFonts, 1500);
    comnputer_element.addEventListener('mouseover', () => {
        clearInterval(rotationInterval);
    });
    
    comnputer_element.addEventListener('mouseout', () => {
        rotationInterval = setInterval(rotateFonts, 1500);
    });
}
